...
from utils import discretize, _get_padded_features, load_data, compute_brier_competing
...
base_data = load_data('/home/arnott/git/CKD-JA-MMSc-1/Results/LossExperiment/Notebook/base_data.json')
...
training_params =  {"alpha": 0.15598768146289432, # rank loss
                    "beta":  1.2, # NLL loss 
                    "delta": 0.05538145469419971,   # longit loss
...
optimizer = torch.optim.AdamW(
    dynamic_deephit_model.parameters(),
    lr=learning_rate,                 # ↓ from ~3.26e-3
    weight_decay=3e-4,       # a touch lighter than 1e-3
    betas=(0.9, 0.99),       # smoother than default 0.999 for beta2
    eps=1e-7,
    amsgrad=True             # helps with rare spikes
)
warmup_steps = 500
global_step = 0
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    ...
    mode='min',
    factor=0.5,              
    patience=40,           #No of Epochs with no improvement after which LR reduced   
    threshold=1e-4,         #
    threshold_mode='rel',
    cooldown=10, # Number of epochs to wait before resuming normal after reduction
    min_lr=1e-6
)
...
# base_data = load_data('/home/arnott/git/CKD-JA-MMSc-1/Results/LossExperiment/NotebookMod/base_data_1.json')
fig, ax = plt.subplots(2,2, figsize = (15,5))
# ax[0,0].plot(base_data['train_epoch_losses'], label='Train DDH')
ax[0,0].plot(train_epoch_losses, label='Train DDH')
...
ax[0,1].plot(val_sel_epoch_losses ,label='validation DDH')
...
ax[1,0].plot(val_nll_epoch_losses ,label='nll validation DDH')
...
ax[1,1].plot(val_longit_epoch_losses ,label='nll validation DDH')
...
plt.tight_layout()
# plt.savefig(f'/home/arnott/git/CKD-JA-MMSc-1/Results/LossExperiment/Notebook/base_loss_1.png')

plt.savefig(f'/home/arnott/git/CKD-JA-MMSc-1/Results/LossExperiment/NotebookMod1/Plots/run_{run}_plot.png')


data = {'train_epoch_losses':train_epoch_losses,
...
# with open(f'/home/arnott/git/CKD-JA-MMSc-1/Results/LossExperiment/NotebookMod/run_{run}_params.json', "w") as f:
#     json.dump(data, f, indent=2)


with open(f'/home/arnott/git/CKD-JA-MMSc-1/Results/LossExperiment/NotebookMod1/Params/base_data_run_{run}.json', "w") as f:     
    json.dump(data, f, indent=2)
