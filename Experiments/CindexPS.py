...
best_avg = float('inf')
threshold1 = 0.1  # Model Loss
threshold2 = 0.05  # Domain Loss
threshold3 = 0.001  # NLL Loss
...
training_params =  {"alpha": 0.30598768146289432, # rank loss
                "beta":  1, # NLL loss 
                "delta": 0.08538145469419971,   # longit loss
                "sigma": 0.20434828698561155,
...
base_brier = [np.float64(0.22032687142828292), np.float64(0.23958305563686008), np.float64(0.2150899685620446), np.float64(0.12031872348288367)]
base_cindex = [np.float64(0.692), np.float64(0.6707259953161593), np.float64(0.6402493321460374), np.float64(0.5788102475032566)]
...
search_space = {
    "gamma_1":{"type": "uniform", "low": 0.25, "high": 1},
...
for j in range(trials):
    ...
    if (np.array(base_brier) > np.array(brier_scores['death'])).sum() == len(brier_scores['death']):
        with open(f'/home/arnott/git/CKD-JA-MMSc-1/Results/Brier_eGFR/Config_run_{run}_{j}.json', "w") as f:
            cfg['brier'] = brier_scores['death']
            json.dump(cfg, f, indent=2)

    if (np.array(base_cindex) < np.array(concordance_scores['death'])).sum() == len(concordance_scores['death']):
        with open(f'/home/arnott/git/CKD-JA-MMSc-1/Results/cindex_eGFR/Config_run_{run}_{j}.json', "w") as f:
            cfg['cindex'] = concordance_scores['death']
            json.dump(cfg, f, indent=2)
