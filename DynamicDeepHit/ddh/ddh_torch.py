...
class DynamicDeepHitTorch(nn.Module):
	...
	def __init__(self, input_dim, output_dim, layers_rnn,
		...
		if self.typ == 'LSTM':
			self.embedding = nn.LSTM(input_dim, hidden_rnn, layers_rnn,
								bias=False, batch_first=True,dropout=0.1)
		if self.typ == 'RNN':
	...
	def forward(self, x):
		...
		x = x.clone()
		inputmask = torch.isnan(x)
		x[inputmask] = 0
		hidden, _ = self.embedding(x)		
		inputmask_1 = torch.isnan(x[:,:,0])
		...
		last_observations = ((~inputmask_1).sum(axis = 1) - 1)
...
# copied over from DynamicDeepHit/ddh/utils.py by GHC
