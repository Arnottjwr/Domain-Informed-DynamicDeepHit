...
def domain_loss_2(longitudinal_prediction, bound_dict): ##TODO - Find some upper and lower bounds
    ...
    return lower_loss.mean(), [lower_loss[:, :, k].mean() for k in range(lower_loss.shape[2])]
...
def domain_loss_3(longitudinal_prediction, bound_dict): ##TODO - Find some upper and lower bounds
    ...
    return upper_loss.mean(), [upper_loss[:, :, k].mean() for k in range(upper_loss.shape[2])]
...
def total_loss(model, x, t, e, training_params): #x: X_batch, t: Y_batch, e: D_batch
    ...
    dl2, dl2_arr = domain_loss_2(longitudinal_prediction,bound_dict)
    dl3, dl3_arr = domain_loss_3(longitudinal_prediction,bound_dict)

    model_loss = delta * longit_loss + alpha * rank_loss + beta * nll_loss 

    if use_constraints:
        domain_loss = (gamma_1 * dl1) + (gamma_2 * dl2) + (gamma_3 * dl3)
        total_losses = model_loss + domain_loss
        return total_losses, (dl1, dl2, dl3, dl2_arr, dl3_arr, model_loss)
    
    return model_loss, (dl1, dl2, dl3, dl2_arr, dl3_arr, model_loss)
