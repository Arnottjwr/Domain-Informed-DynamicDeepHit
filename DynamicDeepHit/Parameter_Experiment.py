...
best_avg = float('inf')
threshold1 = 0.05  # Model Loss
threshold2 = 0.005  # Domain Loss
threshold3 = 0.0001  # NLL Loss



gamma_1s =  np.linspace(0,2,5)
gamma_2s =  np.linspace(0,1,3)
gamma_3s =  np.linspace(0,1,3)
all_configs = list(itertools.product(gamma_1s,gamma_2s, gamma_3s))
results = pd.DataFrame(columns=['gamma_1','gamma_2','gamma_3', 'Epochs for model convergence','Epochs for domain convergence','Epochs for nll convergence',\
                                 'Brier','C-index','Test Loss Total','Test Loss NLL', 'DL1 Mean Violation Rate','DL2 Mean Violation Rate','DL3 Mean Violation Rate'])
...
for j, (gamma_1, gamma_2,gamma_3)  in enumerate(all_configs):
    print(f'gamma 1: {gamma_1}')
    print(f'gamma 2: {gamma_2}')
    print(f'gamma 3: {gamma_3}')

    training_params['gamma'] = [gamma_1,gamma_2,gamma_3, 0]
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if tr_log['model'] < threshold1:
                model_loss_epoch = epoch + 1
        
            if tr_log['domain'] < threshold2:
                domain_loss_epoch = epoch + 1

            if tr_log['nll'] < threshold3:
                nll_loss_epoch = epoch + 1
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(f'/home/arnott/git/CKD-JA-MMSc-1/Results/ParameterExperiment/FinalRun/{gamma_1}_{gamma_2}_{gamma_3}_Loss_plot_run{run}.png')
    pd.Series(model_losses).to_csv(f'/home/arnott/git/CKD-JA-MMSc-1/Results/ParameterExperiment/FinalRun/{gamma_1}_{gamma_2}_{gamma_3}_model_losses__run{run}.csv')
    pd.Series(domain_losses).to_csv(f'/home/arnott/git/CKD-JA-MMSc-1/Results/ParameterExperiment/FinalRun/{gamma_1}_{gamma_2}_{gamma_3}_domain_losses_run{run}.csv')
    ...
    if model_loss_epoch == None:
        model_loss_epoch = epoch + 1

    if domain_loss_epoch == None:
        domain_loss_epoch = epoch + 1

    if nll_loss_epoch == None:
        nll_loss_epoch = epoch + 1

    results.loc[j] = [gamma_1, gamma_2, gamma_3, model_loss_epoch, domain_loss_epoch,nll_loss_epoch, np.mean(brier_scores['death']),\
                      np.mean(concordance_scores['death']),test_total_loss,test_nll_loss,\
                        np.mean(dl1_pct_violate), np.mean(dl2_pct_violate), np.mean(dl3_pct_violate)]
    print(results)
# results.to_csv(f'/home/arnott/git/CKD-JA-MMSc-1/Results/ParameterExperiment/Real_eGFR/ParameterExperiment_Run_{run}.csv')
results.to_csv(f'/home/arnott/git/CKD-JA-MMSc-1/Results/ParameterExperiment/FinalRun/ParameterExperiment_Run_{run}.csv')
print('Results Saved')
